...
@Component
public class ValidacionImagenPrendaImpl implements IValidacionImagenPrenda {
    ...
    public void validarTamano(ImagenPrenda imagenPrenda) {
        if (imagenPrenda.getTamano_KB() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "El tamaño de la imagen (KB) es obligatorio.");
        }
        if (imagenPrenda.getTamano_KB() <= 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "El tamaño de la imagen debe ser mayor a 0 KB.");
        }
        if (imagenPrenda.getTamano_KB() > TAMANO_MAXIMO_KB) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
